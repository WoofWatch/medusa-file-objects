...
class VultrObjectService extends AbstractFileService {
  constructor({}, options) {
    super({}, options)

    this.bucket_ = options.bucket
    this.storageUrl = options.storage_url?.replace(/\/$/, "")
  ...
  uploadFile(file, options = { isProtected: false, acl: undefined }) {
    ...
    return new Promise((resolve, reject) => {
      s3.upload(params, (err, data) => {
        ...
        if (this.storageUrl) {
          resolve({ url: `${this.storageUrl}/${data.Key}`, key: data.Key })
        }
...
export default VultrObjectService
